...
class SampleAreaPoses:
    def __init__(self, node_name):
        ...
        self.map_dims = rospy.get_param("/map_dims") #tuple, LxW map dimensions
        self.orig_x, self.orig_y = rospy.get_param("/origin") #origin of the map

        # Initialize variables
        self.graph = None
        self.bag_of_poses = [] #Initialize bag of poses potential for sampling
    ...
    def sample_poses_quadrants(self, bag_of_poses, map_dims, nsample):
        """
        Samples (x, y) poses for each quadrant in the map, given its LxW Cartesian coordinates

        Inputs:
          bag_of_poses - list of tuples, bag of (x, y) poses in which to sample poses from #TODO: We can either call this in the method instead an input, will check during ROS integration
          map_dims - tuple, dimensions of the map (L, W)
          nsample - scalar, total number of areas to sample. (Ideally, should be divisible by 4)

        """
        L, W = map_dims  # length and width dimensions of the map
        quadrant_bags = [[], [], [], []]  # bags for each quadrant to choose from
        quadrant_samples = list()  # samples from each quadrant bag
        assert (nsample % 4 == 0) and (nsample > 1), "Total areas should be divisble by 4 and greater than 1!"
        sample_size = nsample // 4  # sample size per quadrant
        # self.debug("Bag of poses: {}".format(bag_of_poses))
        # Segregate into quadrant bags
        for (x, y) in bag_of_poses:
            q = None
            if (x > self.orig_x) and (x < self.orig_x + (L/2)):
                if y < self.orig_y + W/2:  # Quadrant 3
                    q = 3
                elif y > self.orig_y + W/2:  # Q2
                    q = 2
            elif x > self.orig_x + L/2:
                if y < self.orig_y + W/2:  # Q4
                    q = 4
                elif y > self.orig_y + W/2:
                    q = 1
            if q is not None:
                quadrant_bags[q - 1].append((x, y))

        for quad in range(4):
            # self.debug("Quadrant {} bag: {}".format(quad+1, quadrant_bags[quad]))
            if len(quadrant_bags[quad]) > 0:  # The sample size or whatever there is in the quadrant
                r = min(sample_size, len(quadrant_bags[quad]))
                sampled = rd.sample(quadrant_bags[quad], r)
            else:
                sampled = [] # Not enough subpopn to sample, return empty quadrant sample
            # self.debug("Sampled: {}".format(sampled))
            quadrant_samples.append(sampled)
        return sum(quadrant_samples, [])
    ...
    def sample_nodes_from_voronoi(self, nsample, seed):
        ...
        if len(self.bag_of_poses) == 0 or self.bag_of_poses is None:
            for node in potential_nodes.indices:
                p = self.graph.vs[node]["coord"]
                p_t = self.latest_map.grid_to_pose(p)
                p_coords = (p_t[0], p_t[1])
                self.bag_of_poses.append(p_coords) #What if this is fixed. Since this is only once

        #Step 2: Sample from each quadrant
        result = self.sample_poses_quadrants(self.bag_of_poses, self.map_dims, nsample)

        return result

    """
    How to test this part?
        > Run the sampling of nodes part in the run experiment
        > Check whether:
            1. We get the correct number of samples per quadrant amounting to the total
            2. And the coords of those samples per quadrant are correct
    """
    ...
    def sample_poses(self):
        ...
        for p in range(self.nplacements):
            ...
            sampled_nodes = self.sample_nodes_from_voronoi(self.nareas, seed)
            self.sampled_nodes_poses['n{}_p{}'.format(self.nareas, p+1)] = sampled_nodes
            self.debug("Placement {}, sampled nodes: {}".format(p, sampled_nodes))
