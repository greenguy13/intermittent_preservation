...
"""
Correlated Multi-armed Thompson Sampling
...
class Robot:
    ...
    def update_mean_loss(self, area):
        ...
        #Do the same for the taus
        curr_tau = self.taus[area-1] #zero-indexed
        self.taus[area-1] = self.counts_visited[area-1] / self.sigma ** 2 #zero-indexed

        # self.debug('Recorded losses: {}. Current mean: {}. New mean: {}'.format(self.recorded_losses[area], mean_loss,
        #                                                                         new_mean_loss))
        # self.debug('Recorded tau: {}. New tau: {}'.format(curr_tau, self.taus[area-1]))
    ...
    def greedy_best_decision(self):
        ...
        """
        Part 2: Apply Thompson sampling among the competitive arms
        """
        ...
        for decision in competitive_arms:
            ...
            if not prune(self.battery, battery_consumption, self.battery_reserve) and decision != self.curr_loc:
                ...
                #                                                                                feasible_battery))
                decision_array.append((decision, min_mean, feasible_battery))
