...
class Robot:
    def __init__(self, node_name):
        ...
        self.max_battery = rospy.get_param("~max_battery") #rospy.get_param("/max_battery") #Max battery
    ...
    #TODO: Could it be we have a method that cancels area_assignment?
    ...
    def go_to_target(self, goal_idx):
        ...
        :return:
        """
        goal = self.sampled_nodes_poses[goal_idx] #Here: 0 would be charging station
    ...
    #TODO: Distance matrix is provided
    ...
    def instantiate_variables(self, assigned_areas, decay_rates, tlapses, dist_matrix):
        ...
        self.debug("Subscribed area fmeasures: {}".format(self.curr_fmeasures))
        ...
        if self.robot_id < 999: #Not a dummy robot
            ...
            while not rospy.is_shutdown():
                ...
                if self.robot_status == robotStatus.IDLE.value: # Here, robot is available but unassigned
                    ...
                    self.debug('Best decision: {}. idx: {}. Process time: {}s'.format(self.get_assigned_area_id(self.best_decision_idx), self.best_decision_idx, think_elapsed))
    ...
    def notify_assignment_accomplishment(self, area_id):
        """
        Notifies central of recent goal accomplishment
        :param clusters:
        :return:
        """
...
#### Version prior to re-structuring
...
#
#                 # TODO: It is here where we edit, we don't reset the f-measure of the area we visit
#                 updated_fmeasures = self.adjust_fmeasures(self.curr_fmeasures.copy(), decision_idx,
...
#                                                                    (self.fsafe, self.fcrit),
#                                                                    self.gamma, self.dec_steps,
#                                                                    mean_duration_decay_dict)  # forecasted opportunity cost
#
#                 # self.debug("Discounted future losses through {} steps: {}".format(self.dec_steps, forecasted_cost_decision))
#                 evaluated_cost_decision = immediate_cost_decision + forecasted_cost_decision  # total forecasted opportunity cost
#                 # self.debug("Appending: {}".format((decision, evaluated_cost_decision, feasible_battery)))
...
#             self.subscribe_statuses[area_id] = rospy.Subscriber('/area_{}/status'.format(area_id), Int8,
#                                                                 self.area_status_cb, area_id)
#
#         # Wait for self.curr_fmeasures to be populated
#         while set(list(self.curr_fmeasures.keys())) != set(self.areas):
#             self.debug("Subscribing to area topics...")
...
#
#                 # Update tlapse for each area when all nodes have registered
#                 if len(self.decisions_made) > 1 or (self.robot_status != robotStatus.IDLE.value) and (
#                         self.robot_status != robotStatus.READY.value):
