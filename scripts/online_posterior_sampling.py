...
"""
Online Posterior Sampling for POMDP under STAR settings

Papers:
1. Posterior Sampling-based Online Learning for Episodic POMDPs, Tang, et al., 2024. https://doi.org/10.48550/arXiv.2310.10107
2. (More) Efficient Reinforcement Learning via Posterior Sampling, Osband, et al., NIPS 2013. https://papers.nips.cc/paper_files/paper/2013/file/6a5889bb0190d0211a991f47bb19a777-Paper.pdf
"""
import rospy
...
class Robot:
    ...
    def action_send_done_cb(self, state, result, robot_id):
        ...
        if state == SUCCEEDED:
            ...
            if self.mission_area_idx == self.charging_station:
                ...
                data = request_fmeasure(mission_area_id)
                measured_f = float(data)
                self.recorded_fdata[self.curr_loc_idx].append(measured_f)
                observed_decay_rate = get_decay_rate(self.max_fmeasure, measured_f, self.tlapses[self.curr_loc_idx])
                self.observations_dict[self.curr_loc_idx].append(observed_decay_rate)

                self.debug("Prior belief decay distribution: {}".format(self.posterior_belief_disbn[self.curr_loc_idx]))
                self.bayesian_belief_update(self.curr_loc_idx)
                self.debug("Observed decay rate: {}. Decay rate history: {}. Updated posterior decay distribution: {}".format(
                        observed_decay_rate, self.observations_dict[self.curr_loc_idx],
                        self.posterior_belief_disbn[self.curr_loc_idx]))
    ...
    def build_pomdp(self):
        ...
        STAR = STARProblem(init_true_state, init_belief, policy_model=policy_model, transition_model=transition_model,
                           ...
                           env_transition_model=env_transition_model)
    ...
    def bayesian_belief_update(self, area_idx):
        """
        ...
        """
        #Prior
        prior_mean, prior_sd = self.posterior_belief_disbn[area_idx] #list of current posterior belief per area, now set as prior

        #Empirical mean and sd
        n = len(self.observations_dict[area_idx])
        mean_data = np.mean(self.observations_dict[area_idx]) #list of observations per area
        sd_data = np.std(self.observations_dict[area_idx], ddof=1)
    ...
    def sample_posterior_belief(self, area_idx):
        ...
        mean_belief, sd_belief = self.posterior_belief_disbn[area_idx]
    ...
    def get_assigned_area_index(self, area_id):
        ...
        Note: area_idx is personal indexing of the robot. While, area_id is the actual area id in the environment
        :param area_id:
        :return:
        """
    ...
    def get_assigned_area_id(self, area_idx):
        """
        Gets actual assigned area id given area index, (area index starts at 1)
        Note: area_idx is personal indexing of the robot. While, area_id is the actual area id in the environment
        :param self:
        :param area_idx:
        :return:
    ...
    def run_operation(self, filename, freq=1):
        ...
        if self.robot_id < 999: #Not a dummy robot
            ...
            # Posterior sample
            for area_idx in self.areas:
                belief_decay = self.sample_posterior_belief(area_idx) #Sample posterior belief based on prior knowledge
                self.decay_rates_dict[area_idx] = belief_decay
                self.observations_dict[area_idx].append(belief_decay) #Append to history of observed decay, (set as initial)
            ...
            while not rospy.is_shutdown():
                ...
                if self.robot_status == robotStatus.IDLE.value: # Here, robot is available but unassigned
                    ...
                    for area_idx in self.areas:
                        belief_decay = self.sample_posterior_belief(area_idx)
                        self.decay_rates_dict[area_idx] = belief_decay
                ...
                if len(self.decisions_made)>0 or (self.robot_status != robotStatus.IDLE.value) and (self.robot_status != robotStatus.READY.value) and (self.robot_status != robotStatus.CONSIDER_REPLAN.value):
                    self.update_tlapses_areas()  # Update the tlapse per area
                    self.compute_curr_fmeasures() # Update belief fmeasures

                if self.save:
                    if self.inference is not None:
                        pu.dump_data((self.recorded_fdata, self.observations_dict), '{}_robot{}_recorded_data'.format(filename, self.robot_id))
    ...
    def compute_curr_fmeasures(self):
        ...
        """
        for area_idx in self.areas:
           self.curr_fmeasures[area_idx] = decay(self.decay_rates_dict[area_idx], self.tlapses[area_idx], self.max_fmeasure)
